...
export type LexDef =
  | { type: 'record'; key: string; description?: string; record: LexObject }
  | LexObject;
...
export type LexProp =
  | { type: 'string'; description?: string; format?: string; maxLength?: number; knownValues?: string[] }
  | { type: 'integer'; description?: string; minimum?: number; maximum?: number }
  | { type: 'boolean'; description?: string }
  | { type: 'array'; description?: string; items: LexProp; minLength?: number; maxLength?: number }
  | { type: 'ref'; description?: string; ref: string }
  | { type: 'unknown'; description?: string };

/** A local id (`char.emma`, `place.mcgolrick-market`, `scene.book1.ch1`). */
const localId = (description: string): LexProp => ({
  type: 'string',
  description,
  maxLength: 256,
});
...
export function buildLexiconDocs(nsid: string): LexiconDoc[] {
  return [scene(nsid), stateEvent(nsid), profile(nsid), place(nsid), item(nsid), custodyEvent(nsid)];
}
        ...
        record: {
          ...
          properties: {
            ...
            sequence: {
              type: 'integer',
              description:
                'The sequence within the book that this scene belongs to — the grouping between book and chapter. Sourced from the frontmatter key named by `project.sequenceField`.',
              minimum: 0,
            },
            ...
            createdAt: {
              type: 'string',
              description:
                'Story time as an RFC3339 datetime (midnight UTC on `storyDate`). Fictional records are ordered by story time, not authoring time.',
              format: 'datetime',
            },
            sourceFile: {
              type: 'string',
...
function stateEvent(ns: string): LexiconDoc {
  return {
    lexicon: 1,
...
function profile(ns: string): LexiconDoc {
  return {
    lexicon: 1,
...
function place(ns: string): LexiconDoc {
  return {
    lexicon: 1,
    id: `${ns}.place`,
...
function item(ns: string): LexiconDoc {
  return {
    lexicon: 1,
    id: `${ns}.item`,
        ...
        record: {
          ...
          properties: {
            ...
            firstAppearance: {
              type: 'string',
              description:
                'Chapter where the item first changes hands, e.g. `book1#ch1`. Derived from the earliest custody event, so it is absent for an item with no custody recorded yet.',
              maxLength: 256,
            },
            sourceFile: {
              type: 'string',
...
function custodyEvent(ns: string): LexiconDoc {
  return {
    lexicon: 1,
    id: `${ns}.custodyEvent`,
      ...
      main: {
        ...
        record: {
          ...
          properties: {
            ...
            fromHolder: localId(
              'Registry id of the previous holder. Absent when the item enters the story here, or when the prior holder is deliberately unnamed — never null.'
            ),
            event: {
              type: 'string',
