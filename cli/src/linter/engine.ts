...
export class LinterEngine {
    ...
    for (const file of files) {
      ...
      if (Array.isArray(data.custody)) {
        for (const raw of data.custody) {
          ...
          if (!itemName || !holderName) continue;
          custody.push({
            item: itemName,
            holder: holderName,
   ...
   */
  private splitBody(content: string): string {
    const lines = content.split(/\r?\n/);
   ...
   */
  public loadPosts(storyDir: string): PostSource[] {
  ...
  public lint(): Diagnostic[] {
    ...
    for (const storyDir of stories) {
      ...
      if (this.config.rules['co-presence-conflict'] !== 'off') {
        ...
        const resolvedPlaceIds = (fmLocation: unknown): string[] => {
          const locs = fmLocation ? (Array.isArray(fmLocation) ? fmLocation : [fmLocation]) : [];
          return locs
            .map((l: unknown) => this.registry.resolve(String(l), 'place')?.id)
        ...
        for (let i = 0; i < chapters.length; i++) {
          ...
          if (placeIdsA.length === 0) continue;
          ...
          for (let j = i + 1; j < chapters.length; j++) {
            ...
            if (overlaps) {
              ...
              const hasSharedLocation = placeIdsA.some((id: string) => placeIdsB.includes(id));
      ...
      if (this.config.rules['post-register'] !== 'off') {
        const severity = (this.config.rules['post-register'] ?? 'error') as 'error' | 'warning';
        ...
        if (posts.length > 0) {
          ...
          const registerAt = new Map<string, string>();
          for (const ch of chapters) {
          ...
          for (const post of posts) {
            ...
            if (!author) continue; // the compiler reports an unresolvable author
            ...
            if (!safe.includes(register)) {
              diagnostics.push({
                ...
                severity,
                message:
                  `Post is anchored to chapter ${chapter}, where ${author.id} is in the ` +
